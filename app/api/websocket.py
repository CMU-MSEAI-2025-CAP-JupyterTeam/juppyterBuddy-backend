...
async def process_message(
    message: Dict[str, Any], session_id: str, notebook_context: Dict[str, Any] = None
):
    """
    ...
    if session_id not in conversation_history:
        conversation_history[session_id] = []  # Initialize empty history

    # Add user message to history
    conversation_history[session_id].append({"role": "user", "content": content})
    ...
    if notebook_context:
        ...
        context_message = (
            f"Current notebook: {notebook_context.get('title', 'Untitled')}\n"
        )
...
async def websocket_endpoint(websocket: WebSocket, session_id: str):
    ...
    try:
        ...
        try:
            await websocket.send_json(
                {"type": "system", "content": f"Error: {str(e)}", "id": str(uuid4())}
            )
        except:
            pass
    finally:
