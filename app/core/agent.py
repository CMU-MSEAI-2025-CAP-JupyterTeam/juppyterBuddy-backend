...
class AgentState(TypedDict):
    ...
    notebook_context: Optional[Dict[str, Any]] # Stores notebook metadata (cells, variables, etc.)
    messages: List[Union[SystemMessage, HumanMessage, AIMessage, FunctionMessage]] # Chat history
    next_steps: List[Dict[str, Any]] # Actions the agent wants to perform (e.g., create cell)
    tool_result: Optional[str] # Stores results of executed tools (if any)
    output_to_user: Optional[str] # Final response from the agent
    should_continue: bool # Determines if LangGraph should keep running
...
class JupyterBuddyAgent:
    ...
    # handle_message method to handle new messages from the user
    def handle_message(
        self, user_message: str, notebook_context: Optional[Dict[str, Any]] = None
    ):
        """
        ...
        formatted_context = (
            ...
            else json.dumps(notebook_context, indent=2)
        )

        # Create system message
        system_message = SystemMessage(
            content=SYSTEM_PROMPT.format(notebook_context=formatted_context)
        )
    ...
    def handle_action_result(
        ...
        state = {
            ...
            "notebook_context": notebook_context,
            "messages": [system_message, user_msg, function_msg],
            "next_steps": [],
