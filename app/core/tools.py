...
class CreateCellInput(BaseModel):
    ...
    position: CellPosition = Field(
        ...
        description="Position to insert the cell ('start', 'end', 'after_active', 'before_active')"
    )
...
def create_cell_tool(
    cell_type: CellType,
    content: str,
    position: CellPosition = CellPosition.AFTER_ACTIVE
) -> Dict[str, Any]:
    """
    ...
    Args:
        ...
        position: Where to insert the cell ('start', 'end', 'after_active', 'before_active')
        
    Returns:
        Dictionary with action_type and payload for the frontend
    """
    logger.info(f"Creating cell: type={cell_type}, position={position}")
    return {
...
def update_cell_tool(cell_index: int, content: str) -> Dict[str, Any]:
    ...
    Returns:
        Dictionary with action_type and payload for the frontend
    """
    logger.info(f"Updating cell: index={cell_index}, new_content_length={len(content)}")
    return {
...
def execute_cell_tool(cell_index: int) -> Dict[str, Any]:
    ...
    Returns:
        Dictionary with action_type and payload for the frontend
    """
    logger.info(f"Executing cell: index={cell_index}")
    return {
...
def get_notebook_info_tool(include_cell_content: bool = True) -> Dict[str, Any]:
    ...
    Returns:
        Dictionary with action_type and payload for the frontend
    """
    logger.info(f"Retrieving notebook info: include_cell_content={include_cell_content}")
    return {
